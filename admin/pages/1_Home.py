...
from core.crud import get_blocks_by_section, update_block, create_block, delete_block
...
def rich_text_editor(key: str, initial_content: str = "", height: int = 200) -> str:
    """Embed a TinyMCE rich text editor using st-tiny-editor."""
    if tiny_editor is None:
        st.error(
            "Missing dependency: `st-tiny-editor`. Please run: `pip install st-tiny-editor`"
        )
        return st.text_area(
            "Content (Editor unavailable)", value=initial_content, key=f"{key}_fallback"
        )

    # Initialize session state for this editor
    if f"{key}_html" not in st.session_state:
        st.session_state[f"{key}_html"] = initial_content

    # Initialize textarea session state to match html state
    if f"{key}_textarea" not in st.session_state:
        st.session_state[f"{key}_textarea"] = st.session_state[f"{key}_html"]

    # Render the TinyMCE editor
    # This component returns the content automatically on change or blur
    editor_content = tiny_editor(
        apiKey="xwjh0uelpw95wzycso924hn13ol42bmhgvtwvr2nlyq5fc8z",
        key=f"editor_{key}",
        initial_value=st.session_state[f"{key}_html"],
        height=height + 150,
    ...
    # Sync editor result back to session state
    if editor_content is not None and editor_content != st.session_state.get(f"{key}_html", ""):
        st.session_state[f"{key}_html"] = editor_content
        st.session_state[f"{key}_textarea"] = editor_content

    # Text area showing current source content
    # Use only the key parameter to avoid conflict with session state
    html_content = st.text_area(
        "HTML Source (updates automatically)",
        key=f"{key}_textarea",
        height=150,
    )

    # Update session state if text area was manually edited
    current_html = st.session_state.get(f"{key}_html", "")
    if html_content != current_html:
        st.session_state[f"{key}_html"] = html_content

    return st.session_state.get(f"{key}_html", initial_content)


st.set_page_config(page_title="Home Content Editor", page_icon="🏠", layout="wide")
...
with tab1:
    ...
    cta_secondary_block = next(
        (b for b in blocks if b.type == "hero_cta_secondary"), None
    )
    ...
    with st.expander("Headline", expanded=True):
        if headline_block:
            ...
            line2 = st.text_input(
                "Line 2 (Italic)", value=headline_block.content.line2_italic
            )
            if st.button("Update Headline", key="update_headline"):
                ...
                update_block(
                    headline_block.id,
                    content=HeroHeadlineContent(
                        line1=line1, line2_italic=line2, full_html=full_html
                    ),
                )
    ...
    with st.expander("Description", expanded=True):
        st.markdown(
            "**Rich Text Editor** - Use the toolbar to format text (bold, italic, links, lists)"
        )

        if description_block:
            # Get initial HTML content if it exists, otherwise use text
            initial_html = (
                getattr(description_block.content, "html", None)
                or description_block.content.text
            )

            # Rich text editor
            html_content = rich_text_editor(
                key="hero_desc", initial_content=initial_html, height=200
            )
            ...
            with col1:
                if st.button(
                    "✅ Update Description", key="update_hero_desc", type="primary"
                ):
                    # Store both text and html versions
                    update_block(
                        description_block.id,
                        content=HeroDescriptionContent(
                            text=plain_text, html=html_content
                        ),
                    )
            ...
            with col2:
                if st.button(
                    "🗑️ Clear",
                    key="clear_hero_desc",
                    help="Clears the editor and the text area.",
                ):
                    # Clear the session state values instead of deleting keys
                    st.session_state[f"hero_desc_html"] = ""
                    st.session_state[f"hero_desc_textarea"] = ""
                    if f"editor_hero_desc" in st.session_state:
                        st.session_state[f"editor_hero_desc"] = ""
                    st.rerun()
        else:
            # Rich text editor for new description
            html_content = rich_text_editor(
                key="hero_desc_new", initial_content="", height=200
            )
            ...
            if st.button(
                "✅ Create Description", key="create_hero_desc", type="primary"
            ):
                create_block(
                    "hero_description",
                    HeroDescriptionContent(text=plain_text, html=html_content),
                )
    ...
    with col1:
        with st.expander("Primary CTA", expanded=True):
            if cta_primary_block:
                label = st.text_input(
                    "Button Text", value=cta_primary_block.content.label
                )
                ...
                if st.button("Update Primary CTA", key="update_primary_cta"):
                    update_block(
                        cta_primary_block.id,
                        content=HeroCtaContent(label=label, url=url, icon=icon),
                    )
                ...
                if st.button("Create Primary CTA", key="create_primary_cta"):
                    create_block(
                        "hero_cta_primary",
                        HeroCtaContent(label=label, url=url, icon=icon),
                    )
    ...
    with col2:
        with st.expander("Secondary CTA", expanded=True):
            if cta_secondary_block:
                label = st.text_input(
                    "Button Text", value=cta_secondary_block.content.label
                )
...
with tab2:
    ...
    if ribbon_block:
        ...
        status = st.text_input(
            "Registration Status", value=ribbon_block.content.registration_status
        )
        info_url = st.text_input(
            "Exam Info URL", value=ribbon_block.content.exam_info_url
        )
        reg_url = st.text_input(
            "Registration URL", value=ribbon_block.content.registration_url
        )

        if st.button("Update Exam Ribbon", key="update_exam"):
            update_block(
                ribbon_block.id,
                content=ExamRibbonContent(
                    exam_date=exam_date,
        ...
        status = st.text_input(
            "Registration Status", value="Registration closes in 12 days"
        )
...
with tab3:
    ...
    description_block = next(
        (b for b in blocks if b.type == "benefits_description"), None
    )
    ...
    with st.expander("Heading", expanded=True):
        if heading_block:
            ...
            line2 = st.text_input(
                "Line 2 (Italic)", value=heading_block.content.line2_italic
            )
            if st.button("Update Benefits Heading", key="update_benefits_heading"):
                full_html = (
                    f'{line1}<br><span class="italic text-secondary">{line2}</span>'
                )
                update_block(
                    heading_block.id,
    ...
    with st.expander("Description", expanded=True):
        ...
        if description_block:
            ...
            # Rich text editor
            html_content = rich_text_editor(
                key="benefits_desc", initial_content=initial_html, height=200
            )
            ...
            with col1:
                if st.button(
                    "✅ Update Description", key="update_benefits_desc", type="primary"
                ):
                    update_block(
                        description_block.id,
                        content=DescriptionContent(text=plain_text, html=html_content),
                    )
            ...
            with col2:
                if st.button(
                    "🗑️ Clear",
                    key="clear_benefits_desc",
                    help="Clears the editor and the text area.",
                ):
                    # Clear the session state values instead of deleting keys
                    st.session_state[f"benefits_desc_html"] = ""
                    st.session_state[f"benefits_desc_textarea"] = ""
                    if f"editor_benefits_desc" in st.session_state:
                        st.session_state[f"editor_benefits_desc"] = ""
                    st.rerun()
        else:
            # Rich text editor for new description
            html_content = rich_text_editor(
                key="benefits_desc_new", initial_content="", height=200
            )
            ...
            if st.button(
                "✅ Create Description", key="create_benefits_desc", type="primary"
            ):
                create_block(
                    "benefits_description",
                    DescriptionContent(text=plain_text, html=html_content),
                )
...
with tab4:
    ...
    for idx, card in enumerate(card_blocks):
        with st.expander(f"Card {idx + 1}: {card.content.title}", expanded=True):
            badge_type = st.text_input(
                "Badge Type", value=card.content.badge_type, key=f"card_{idx}_badge"
            )
            title = st.text_input(
                "Title", value=card.content.title, key=f"card_{idx}_title"
            )
            url = st.text_input("URL", value=card.content.url, key=f"card_{idx}_url")
            icon = st.text_input(
                "Icon", value=card.content.icon, key=f"card_{idx}_icon"
            )
                ...
                if st.button("Update Card", key=f"update_card_{idx}"):
                    update_block(
                        ...
                        content=ResourceCardContent(
                            badge_type=badge_type, title=title, url=url, icon=icon
                        ),
                    )
...
with tab5:
    ...
    description_block = next(
        (b for b in blocks if b.type == "support_description"), None
    )
    ...
    with st.expander("Heading", expanded=True):
        if heading_block:
            ...
            if st.button("Update Support Heading", key="update_support_heading"):
                full_html = (
                    f'{line1}<br><span class="italic font-normal">{line2}</span>'
                )
                update_block(
                    heading_block.id,
                    content=HeadingContent(
    ...
    with st.expander("Description", expanded=True):
        ...
        if description_block:
            ...
            # Rich text editor
            html_content = rich_text_editor(
                key="support_desc", initial_content=initial_html, height=200
            )
            ...
            with col1:
                if st.button(
                    "✅ Update Description", key="update_support_desc", type="primary"
                ):
                    update_block(
                        description_block.id,
                        content=DescriptionContent(text=plain_text, html=html_content),
            ...
            with col2:
                if st.button(
                    "🗑️ Clear",
                    key="clear_support_desc",
                    help="Clears the editor and the text area.",
                ):
                    # Clear the session state values instead of deleting keys
                    st.session_state[f"support_desc_html"] = ""
                    st.session_state[f"support_desc_textarea"] = ""
                    if f"editor_support_desc" in st.session_state:
                        st.session_state[f"editor_support_desc"] = ""
                    st.rerun()
        else:
            # Rich text editor for new description
            html_content = rich_text_editor(
                key="support_desc_new", initial_content="", height=200
            )
            ...
            if st.button(
                "✅ Create Description", key="create_support_desc", type="primary"
            ):
                create_block(
                    "support_description",
                    DescriptionContent(text=plain_text, html=html_content),
    ...
    for idx, contact in enumerate(contact_blocks):
        with st.expander(f"Contact {idx + 1}: {contact.content.label}", expanded=True):
            label = st.text_input(
                "Label", value=contact.content.label, key=f"contact_{idx}_label"
            )
            value = st.text_input(
                "Value", value=contact.content.value, key=f"contact_{idx}_value"
            )
            icon = st.text_input(
                "Icon", value=contact.content.icon, key=f"contact_{idx}_icon"
            )

            if st.button("Update Contact", key=f"update_contact_{idx}"):
                update_block(
                    contact.id,
                    content=SupportContactContent(label=label, value=value, icon=icon),
                )
...
with tab6:
    ...
    with st.expander("Copyright Text", expanded=True):
        if copyright_block:
            text = st.text_area(
                "Copyright Text", value=copyright_block.content.text, height=100
            )
            if st.button("Update Copyright", key="update_copyright"):
                update_block(
                    copyright_block.id, content=FooterCopyrightContent(text=text)
                )
            ...
            text = st.text_area(
                "Copyright Text",
                value="© 2024 National Assessment Test Centre.\nPursuing Academic Excellence.",
                height=100,
            )
            ...
            links_text = st.text_area(
                ...
                value="\n".join(
                    [
                        ...
                        for link in links_block.content.links
                    ]
                ),
                height=150,
            )
