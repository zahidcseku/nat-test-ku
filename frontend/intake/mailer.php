...
// DB-only email template loader (returns null if row missing, then we
// fall back to the legacy buildRegistrationEmail()).
...
/**
 * Build all the pre-built HTML blocks and scalar variables needed to
 * render either a DB-stored template or the legacy fallback body.
 *
 * @return array Map of {placeholder-name} => value.
 */
function buildIntakeEmailVars(array $registration): array {
    $e = static fn($v) => htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8');
    ...
    $infoTable = '<table cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:560px;font-size:14px;">';
    foreach ($rows as $label => $value) {
        $infoTable .= '<tr>'
            . '<td style="border:1px solid #d8dde3;background:#f4f6f8;font-weight:bold;width:38%;">' . $e($label) . '</td>'
            . '<td style="border:1px solid #d8dde3;">' . $e($value) . '</td>'
    ...
    // Online payment options: pay button + QR
    $payLink = SITE_URL . '/payment-retry.html?token=' . ($registration['retry_token'] ?? '');
    $paymentOptionsOnline = '<h3 style="color:#002147;">Pay online</h3>'
        . '<p style="font-size:14px;"><a href="' . $e($payLink) . '" '
        . 'style="display:inline-block;background:#002147;color:#ffffff;padding:10px 22px;border-radius:6px;text-decoration:none;">'
        . 'Complete your payment</a></p>'
        . '<p style="font-size:13px;color:#555;">Or open this link: <a href="' . $e($payLink) . '">' . $e($payLink) . '</a></p>'
    ...
    // Offline payment options: bank details + QR
    $paymentOptionsOffline = '<h3 style="color:#002147;">Pay by bank deposit</h3>'
        . '<table cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-size:14px;">'
        . '<tr><td style="border:1px solid #d8dde3;background:#f4f6f8;font-weight:bold;">Account Name</td><td style="border:1px solid #d8dde3;">' . $e(BANK_ACCOUNT_NAME) . '</td></tr>'
        . '<tr><td style="border:1px solid #d8dde3;background:#f4f6f8;font-weight:bold;">Account Number</td><td style="border:1px solid #d8dde3;">' . $e(BANK_ACCOUNT_NUMBER) . '</td></tr>'
        . '<tr><td style="border:1px solid #d8dde3;background:#f4f6f8;font-weight:bold;">Bank</td><td style="border:1px solid #d8dde3;">' . $e(BANK_NAME) . '</td></tr>'
    ...
    // Banners for the payment_confirmation variants
    $bannerPaid = '<p style="font-size:15px;background:#e6f4ea;border-left:4px solid #1e8e3e;padding:10px 14px;">'
        . '<strong>Payment received.</strong> Your online payment was successful'
        . (!empty($registration['bank_tran_id'])
            ? ' (transaction reference: ' . $e($registration['bank_tran_id']) . ')'
            : '')
        . '.</p>';
    $bannerPending = '<p style="font-size:15px;background:#fef7e0;border-left:4px solid #f9ab00;padding:10px 14px;">'
        . '<strong>Payment receipt received — pending verification.</strong> '
        . 'We received your payment receipt with your application; our team will verify it during review.</p>';
    ...
    return [
        ...
        'payment_options_online' => $paymentOptionsOnline,
        'payment_options_offline'=> $paymentOptionsOffline,
        'banner_paid'            => $bannerPaid,
        'banner_pending'         => $bannerPending,
    ];
}

/**
 * Pick the template_key for a given variant + payment state.
 */
function _pickIntakeTemplateKey(string $variant, array $registration): string {
    $method = $registration['payment_method'] ?? '';
    $status = $registration['payment_status'] ?? '';
    if ($variant === 'submission_receipt') {
    ...
    return ''; // unknown variant → no template, fall back
}

/**
 * Build and send an applicant email. Never throws.
 *
 * Tries the editable DB template first; falls back to the legacy
 * buildRegistrationEmail() if the template row is missing (e.g. before
 * the admin seeds the templates, or if a row was deleted).
 *
...
function sendRegistrationEmail(array $registration, string $variant): bool {
    try {
        $mail = null;
        $key  = _pickIntakeTemplateKey($variant, $registration);
        if ($key !== '') {
            $vars = buildIntakeEmailVars($registration);
            $mail = renderIntakeEmailTemplate($key, $vars);
        }
        if ($mail === null) {
            // Fall back to the legacy hardcoded builder.
