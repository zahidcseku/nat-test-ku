<?php
/**
 * Stage score reports from an xlsx + zip upload.
 *
 * POST /admin/api/scores/upload.php
 ...
 *
 * Redirects back to /admin/pages/scores.php with a flash.
 */
...
if (!$zipValidation['valid']) {
    setFlashMessage('Scores ZIP: ' . $zipValidation['error'], 'error');
    header('Location: ' . BASE_URL . '/pages/scores.php');
    exit;
}
...
$result = stageScoreReportsFromUpload(
    $_FILES['xlsx_file']['tmp_name'],
    $_FILES['scores_zip']['tmp_name'],
    $examDateId,
    $examDate,
    $createdBy
);
...
$msg = "Staged {$result['staged']} score report(s).";
if (!empty($result['warnings'])) {
    $msg .= ' Warnings: ' . implode('; ', array_slice($result['warnings'], 0, 5));
    if (count($result['warnings']) > 5) {
        $msg .= ' (+' . (count($result['warnings']) - 5) . ' more)';
